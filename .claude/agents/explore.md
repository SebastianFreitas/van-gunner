...
- Every script's first `##` line after `extends` is its summary; `docs/PROJECT_MAP.md` lists them per folder. The van is `scenes/van/van.tscn` plus instanced `van_shell.tscn` (46 KB), `van_breach_points.tscn` and `scenes/ui/run_hud.tscn`: grep for node names and read about 40 lines around the hit.
