...
- Read only the region you are changing. For scripts over 300 lines, grep `-n` for the function names the spec gives you, then Read with offset and limit around the hit. `scenes/van/van.tscn` is 87 KB: never read it top to bottom.
...
The usual command is the headless Godot check the spec gives you. Report every output line containing `SCRIPT ERROR`, `Parse Error` or `ERROR:`. Say "clean" only when there are none; the exit code alone is not reliable.
