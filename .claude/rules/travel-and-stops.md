...
- **Lights.** Shadowless `OmniLight3D`, `light_cull_mask = 1`, group `facade_lights`, world cap 12, checked live with an `is_inside_tree()` guard; never on a bay or side-street side. `visibility_range_end` is a `GeometryInstance3D` property: every mesh under a side root gets 64 m, and setting it on a light is a SCRIPT ERROR.
