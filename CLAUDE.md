...
- **Headless check:** `"$GODOT" --headless --path . --editor --quit` from the repo root. `GODOT` is an environment variable holding the full path to `Godot_v4.7-stable_win64_console.exe`. Any output line containing `SCRIPT ERROR`, `Parse Error` or `ERROR:` is a failure; the exit code alone is not reliable.
