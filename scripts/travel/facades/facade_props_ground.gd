...
## docks, arcade pilasters, stoops, sidewalk furniture) and, per side, the wall fixtures and
## their lights. Everything lives on the sidewalk strip or the face and passes the keep-out gate.
	...
	for i in zs.size():
		...
		for plan: Dictionary in plans:
			if z >= float(plan[&"z0"]) and z <= float(plan[&"z1"]):
				...
				break
		var dead := rng.randf() < district.dead_lamp_chance
